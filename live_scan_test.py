...
def make_guard(entities: set[str] | None = None, use_ner: bool = False) -> LLMGuard:
    enabled = entities or {
        ...
        "IBAN", "PHONE", "IP_ADDRESS", "CUSTOM_SECRET",
    }
...
def run(label: str, guard: LLMGuard, text: str) -> object:
    ...
    print(f"  {BOLD}Sanitize:{RESET} {result.sanitized_text[:90]}{'…' if len(result.sanitized_text)>90 else ''}")
    if result.violations:
        for v in result.violations:
            arrow = f"→ '{v.replacement}'" if v.replacement else ""
            print(f"  {YELLOW}  [{v.action.value:4}] {v.entity_type:15} '{v.original}' {arrow}{RESET}")
    else:
...
text = ("Müşterimiz Ali Veli (ali.veli@sirket.com, TC: 12345678901) "
...
text = ("Dear John Smith, your IBAN TR33 0006 1005 1978 6457 8413 26 "
...
for text in clean_texts:
    ...
    check(f"False positive yok: '{text[:45]}'", r.is_clean,
...
text = ("Müşteri Mehmet Demir (mehmet@company.com) 4111111111111111 ile ödedi. "
...
check("PERSON tespit (EN)", "PERSON" in types2, "llama3.1:8b sınırı" if "PERSON" not in types2 else "")
...
check("Email tekrar yok (overlap çözüldü)", len(emails) == 1,
...
for i, (line, res) in enumerate(zip(lines, batch_results), 1):
...
check("Satır 2 temiz",         batch_results[1].is_clean,
      str([v.entity_type for v in batch_results[1].violations]) if not batch_results[1].is_clean else "")
...
check("Hiçbir secret düz kalmadı", all(
    v.replacement is not None
    for v in r.violations if v.entity_type == "CUSTOM_SECRET"
))
...
text = ("hesabıma giremiyorum şifrem calisamiyor!! "
...
text = ("Başvuru sahibi: Deniz Yıldız. "
...
check("Aynı isim → aynı hash",       len(hashes) == 1,
