...
active_regex = sorted(
    e for e in guard._config.get("entities", {})
...
for title, text, ground_truth in CASES:
    ...
    print(f"  {DIM}Metin uzunluğu: {len(text)} karakter — {len(violations)} tespit — ⏱ {elapsed:.1f}s{RESET}\n")
        ...
        if hit_idx is not None:
            ...
            for v in violations:
                if _match(val, v.original) and v.entity_type != etype:
                    ...
                    break
            if mislabel:
                print(f"  {YELLOW}~{RESET} {etype:16} {DIM}{val[:38]:38}{RESET} "
    ...
    if extras:
        ...
        for v in extras:
            det = detector_of(v.confidence)
            print(f"    {YELLOW}+{RESET} {v.entity_type:16} {DIM}'{v.original[:40]}'{RESET} [{det}]")
