...
class TestMiddlewareJsonFieldScan:
    ...
    def test_json_keys_preserved_after_sanitize(self):
        ...
        original = {"user": "user@example.com", "message": "hello", "count": 42}
        ...
        asyncio.run(mw(scope, receive_fn, send_fn))

        result_data = json.loads(sent_body[0])
        # Key "user" preserved
        assert "user" in result_data
        # Email replaced
        assert "user@example.com" not in result_data["user"]
        assert "[EMAIL]" in result_data["user"]
        # Non-PII field unchanged
        assert result_data["message"] == "hello"
