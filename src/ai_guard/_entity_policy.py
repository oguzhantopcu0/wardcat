"""Entity-policy concern of :class:`~ai_guard.AIGuard`, factored out as a mixin.
...
cohesive unit and slims the ``AIGuard`` facade.
"""
...
from ai_guard.core.models import KNOWN_ENTITY_TYPES, Action, Entity, warn_unknown_entity
from ai_guard.core.registry import LAYER_ENTITIES, VALID_LAYERS
from ai_guard.exceptions import ConfigError

logger = logging.getLogger("ai_guard.guard")


class EntityPolicyMixin:
    """Entity configuration + introspection for ``AIGuard``.

    The composing class must provide ``_config`` (the config dict),
    ``_default_action_warned`` (a bool flag), and a ``_rebuild()`` method.
    """

    # Provided by the composing class (AIGuard) — declared for type checkers.
    _config: dict[str, Any]
    _default_action_warned: bool

    def _rebuild(self) -> None:  # pragma: no cover - overridden by AIGuard
    ...
    def add_entity(
        ...
        :param entity_type: An :class:`~ai_guard.Entity` constant (recommended,
                            ...
                            :meth:`remove_entity`.
        :param action:      An :class:`~ai_guard.Action` constant or its string
